...
import com.netease.nim.zcm.DemoPrivatizationConfig;
...
//        ToastHelper.showToast(this, "授权成功");
    ...
    private void login() {
        ...
        loginRequest = NimUIKit.login(new LoginInfo(account, token), new RequestCallback<LoginInfo>() {
            ...
            public void onSuccess(LoginInfo param) {
                LogUtil.i(TAG, "login success"+param.getToken());
                onLoginDone();
    ...
    private void appLogin() {
        ...
        // 注册流程
        final String nickName = loginAccountEdit.getText().toString();
        final String password = loginPasswordEdit.getText().toString();

        ContactHttpClient.getInstance().login( nickName, password, versionName,new ContactHttpClient.ContactHttpCallback<LoginResponse>() {
            @Override
            public void onSuccess(LoginResponse loginResponse) {
                Log.e("登录","登录:"+loginResponse.toString());
                token=loginResponse.getData().getToken();
                if(loginResponse.getRes().equals("300")){
                    downloadUrl = loginResponse.getData1().getUrl();
                    showUpdataDialog();
                }else{
                    if(!isFinishing()) {
                        login();
            ...
            @Override
            public void onFailed(int code, String errorMsg) {
                ToastHelper.showToast(LoginActivity.this, getString(R.string.login_failed, String.valueOf(code), errorMsg));
                DialogMaker.dismissProgressDialog();
            }
        });
    }
...
//        });
    ...
    private boolean checkRegisterContentValid() {
        ...
        if (account.length() <= 7 || account.length() > 20) {
