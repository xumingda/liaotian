...
public class ContactSelectActivity extends UI implements View.OnClickListener, android.support.v7.widget.SearchView.OnQueryTextListener {
    ...
    private Button btnSelect,btnAllSelect;
    ...
    private void initAdapter() {
        ...
        } else if (option.type == ContactSelectType.All) {
            dataProvider = new ContactDataProvider(ItemTypes.TEAM);
        }else {
    ...
    private void initContactSelectArea() {
        ...
        if (option.multi) {
            ...
            if (option.showContactSelectArea) {
                ...
                btnAllSelect.setVisibility(View.VISIBLE);
            } else {
                scrollViewSelected.setVisibility(View.GONE);
                btnSelect.setVisibility(View.GONE);
                btnAllSelect.setVisibility(View.GONE);
            }
    ...
    public void onClick(View v) {
        ...
        }else if(v.getId() == R.id.btnAllSelect){
            if(type==0){
                type=1;
                btnAllSelect.setText("取消全选");
                //全选
                for(int i=0;i<+contactAdapter.getDatas().getCount();i++){
                    LogUtil.e("数据","数据:"+ contactAdapter.getDatas().getItem(i).getItemType());
                    if(contactAdapter.getDatas().getItem(i).getItemType()==1||contactAdapter.getDatas().getItem(i).getItemType()==2){
                        if(!contactAdapter.isSelected(i)){
                            contactAdapter.selectItem(i);
                            ContactItem contactItem=(ContactItem)contactAdapter.getDatas().getItem(i);
                            contactSelectedAdapter.addContact( contactItem.getContact());
                        }
                    }
                }
            }else{
                ...
                for(int i=0;i<+contactAdapter.getDatas().getCount();i++){
                    if(contactAdapter.getDatas().getItem(i).getItemType()==1||contactAdapter.getDatas().getItem(i).getItemType()==2){
                        ...
                        ContactItem contactItem=(ContactItem)contactAdapter.getDatas().getItem(i);
                        contactSelectedAdapter.removeContact( contactItem.getContact());
                    }
                }
            }
