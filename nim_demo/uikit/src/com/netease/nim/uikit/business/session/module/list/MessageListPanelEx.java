...
import com.netease.nim.uikit.business.team.activity.SharedPrefrenceUtils;
...
public class MessageListPanelEx {
    ...
    private HashMap<String, Boolean> selectMsgList;
...
//    private String announceId;
    ...
    MemberChangeAttachment memberChangeAttachment = null;
    ...
    Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            if (msg.what == 1) {
                if (!adapter.isIsok()) {
                    if (items.size() > 0 && !isrequest) {
                        isrequest = true;

                        requestMembers(items.get(0).getSessionId());
                    }
                    //xmd删除移出人的消息
                    for (int j = 0; j < items.size(); j++) {
                        if (items.get(j).getMsgType().toString().equals("notification") && items.get(j).getAttachment() != null) {
                            if (((NotificationAttachment) items.get(j).getAttachment()).getType().toString().equals("KickMember")) {
                                memberChangeAttachment = (MemberChangeAttachment) ((NotificationAttachment) items.get(j).getAttachment());
                            }
                        }
                        if (items.get(j).getMsgType().toString().equals("tip")) {
                            String content = items.get(j).getContent();

                            if (content != null && content.indexOf("deleteMsg@*@") != -1) {
                                deleteGetItem(content.substring(12, content.length()));
                            }
                        }
                    }
                    for (int j = 0; j < items.size(); j++) {
                        if (memberChangeAttachment != null) {
                            for (int i = 0; i < memberChangeAttachment.getTargets().size(); i++) {
                                if (items.get(j).getFromAccount().equals(memberChangeAttachment.getTargets().get(i))) {
                                    deleteItem(items.get(j), true);
                                }
                            }

                        }
                    }
                    memberChangeAttachment = null;
                }
                ...
                boolean noticeClose=SharedPrefrenceUtils.getBoolean(NimUIKit.getContext(),"noticeClose",false);
                //xmd空错误修复
                if(tv_announce!=null){
                    if(noticeClose){
                        tv_announce.setVisibility(View.GONE);
                        ...
                        tv_announce.setOnClickListener(new View.OnClickListener() {
                            ...
                            public void onClick(View view) {
                                AdvancedTeamAnnounceActivity.start(NimUIKit.getContext(), teamId,true);
                            }
                        });
                    }
                }
    ...
    private void initListView(IMMessage anchor) {
        selectMsgList = new HashMap<>();
        ivBackground = rootView.findViewById(R.id.message_activity_background);
        tv_announce = rootView.findViewById(R.id.tv_announce);
        // RecyclerView
        ...
        if(btn_cancle!=null&&btn_delete!=null){
            btn_cancle.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    selectMsgList.clear();
                    adapter.setIsok(false);
            ...
            btn_delete.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    for (int i = 0; i < items.size(); i++) {
                        ...
                        if (selectMsgList.get(items.get(i).getUuid()) != null && selectMsgList.get(items.get(i).getUuid())) {
                            delete_notice(items.get(i));
                            deleteItem(items.get(i), false);
                        }
                    }
    ...
    private OnItemClickListener listener = new OnItemClickListener() {
        ...
        public void onItemClick(IRecyclerView adapter, View view, int position) {
            Log.e("点击","点击一样");
            if (!items.get(position).getMsgType().toString().equals("notification")) {
                if (selectMsgList.get(items.get(position).getUuid()) == null) {
                    selectMsgList.put(items.get(position).getUuid(), true);
                } else {
                    selectMsgList.put(items.get(position).getUuid(), !selectMsgList.get(items.get(position).getUuid()));
                }
            }
...
//            // resend的的情况，可能时间已经变化了，这里要重新检查是否要显示时间
    ...
    private class MsgItemEventListener implements MsgAdapter.ViewHolderEventListener {
        ...
        private void prepareDialogItems(final IMMessage selectedItem, CustomAlertDialog alertDialog) {
            ...
            longClickItemMore(selectedItem, alertDialog);
        }
        ...
        private void longClickItemDelete(final IMMessage selectedItem, CustomAlertDialog alertDialog) {
            ...
            alertDialog.addItem(container.activity.getString(R.string.delete_has_blank), new CustomAlertDialog.onSeparateItemClickListener() {
                ...
                public void onClick() {
                    ...
                    //xmd判断消息是不是自己发的，是就删除本地还有其他端的，若不是判断是不是管理员或者群主，才删除其他端的
                    if (selectedItem.getFromAccount().equals(NimUIKit.getAccount()) ){
                        delete_notice(selectedItem);
                    }else{
                        if (memberList != null && memberList.size() > 0) {
                            for (int i = 0; i < memberList.size(); i++) {
                                if (memberList.get(i).getAccount().equals(NimUIKit.getAccount()) && memberList.get(i).getType() == TeamMemberType.Manager) {
                                    delete_notice(selectedItem);
                                }
                            }
                        }
        ...
        // 长按菜单项--多选
        private void longClickItemMore(final IMMessage selectedItem, CustomAlertDialog alertDialog) {
            if (recordOnly) {
                return;
            }
            //判断是不是群主
            if (creatorId!=null) {
                if(creatorId.equals(NimUIKit.getAccount())){
                    alertDialog.addItem(container.activity.getString(R.string.more_has_blank), new CustomAlertDialog.onSeparateItemClickListener() {

                        @Override
                        public void onClick() {
                            adapter.setIsok(true);
                            if(btn_cancle!=null&&btn_delete!=null) {
...
//                    NIMClient.getService(MsgService.class).revokeMessage(item).setCallback(new RequestCallback<Void>() {
    ...
    //发删除通知
    public void delete_notice(IMMessage msg) {
        // 同时，本地插入被对方拒收的tip消息
        IMMessage tip = MessageBuilder.createTipMessage(msg.getSessionId(), msg.getSessionType());
        //内容
        tip.setContent("deleteMsg@*@" + msg.getUuid());
        tip.setStatus(MsgStatusEnum.success);
        CustomMessageConfig config = new CustomMessageConfig();
        config.enableUnreadCount = false;
        tip.setConfig(config);
        NIMClient.getService(MsgService.class).sendMessage(tip,false);

    }

    // 删除通知收到的消息
    public void deleteGetItem(String uuid) {
        List<IMMessage> messages = new ArrayList<>();
        for (int i = 0; i < items.size(); i++) {
            if (items.get(i).getUuid().equals(uuid)) {
                NIMClient.getService(MsgService.class).deleteChattingHistory(items.get(i));
                adapter.deleteItem(items.get(i), true);
            } else {
        ...
        switch (requestCode) {
            ...
            case REQUEST_CODE_FORWARD_PERSON:
                for (int i = 0; i < selected.size(); i++) {
                    doForwardMessage(selected.get(i), SessionTypeEnum.P2P);
                }
                break;
        }
    ...
    private void requestTeamData() {
        // 请求群信息
        Team t = NimUIKit.getTeamProvider().getTeamById(items.get(0).getSessionId());

        if (t != null) {
            updateAnnounceInfo(t);
            creatorId = t.getCreator();
        } else {
            NimUIKit.getTeamProvider().fetchTeamById(items.get(0).getSessionId(), new SimpleCallback<Team>() {
                @Override
    ...
    private void setAnnounceItem() {
        ...
        List<Announcement> list = AnnouncementHelper.getAnnouncements(items.get(0).getSessionId(), announce, 1);
        if (list == null || list.isEmpty()) {
            return;
        }
        if(tv_announce!=null) {
            tv_announce.setVisibility(View.VISIBLE);
            tv_announce.setText("群公告:" + list.get(0).getContent());
        }
     ...
     * ***************************获取群员信息 ********************************
     */
    private void requestMembers(String teamId) {
